...
public class User {
  public String user_id; // all fields have to be of type String because of type mismatch when loading
                         // from json
  ...
  public User(String user_id, String weight, String bust_size, String height, String age, String body_type,
      String horoscope) {
  ...
  public List<Integer> getUserID(List<List<Number>> neighbors, Collection<User> usersToSearch) {
    ...
    for (List<Number> elt : neighbors) {
      for (User user : usersToSearch) {
        ...
        if ((elt.get(0).equals(weight)) && (elt.get(1).equals(height)) && (elt.get(2).equals(age))) {
          idList.add(i);
        }
      }
    }
   ...
   */
  public List<Integer> returnNeighbors(Collection<User> userList, int numNeighbors, Number weight, Number height,
      Number age) {
   ...
   */
  public HashMap<String, Integer> classify(Collection<User> userList, int numNeighbors, Number weight, Number height,
      Number age) {
    ...
    // parses through IDs of closest users, gets their horoscopes, and adds 1 value
    // of horoscope in hashmap
